...
**产品名已全面统一为 DevBox**：仓库目录名、远端地址（`gitee.com/MeQln/DevBox.git`）、`package.json#name`、`tauri.conf.json`（`productName` / `identifier`）、桌面窗口标题 `DevBox · 开发工具箱`、Cargo `[[bin]] name = "DevBox"` 全部一致，无 fs-tauri 残留。改文案时不要把 DevBox 又改回 fs-tauri。
